...
;(function(jQuery) {
	jQuery.switchable = jQuery.switchable || {};
	...
	function switchTo(triggers, panels, cfg) { 
		...
		// 绑定所有回调函数
		jQuery.each(cfg, function(name, fn) {
		...
		panels.find("a[href^=#]").click(function(e) {
			self.click(jQuery(this).attr("href"), e);		
		}); 
	...
	jQuery.fn.switchable = function(container, cfg) {
		...
		if ( jQuery.isFunction(cfg) ) {
			cfg = { beforeSwitch: cfg };
		}
		
		var globals = jQuery.extend({}, jQuery.switchable.cfg), len = this.length;
		cfg = jQuery.extend(globals, cfg);
		...
		this.each(function(i) {
			...
			if ( !panels.length ) {
				panels = len == 1 ? jQuery(container) : root.parent().find(container);
			}
...
;(function(jQuery) {		
	...
	jQuery.fn.autoplay = function(cfg) { 

		if ( typeof cfg == 'number' ) {
			cfg = { interval: cfg };	
		}
		
		var opts = jQuery.extend({}, t.plugin.autoplay.cfg), ret;
		jQuery.extend(opts, cfg);   	
		
		this.each(function() {		
				
			var api = jQuery(this).switchable();			
			if ( api ) {
				ret = api;
			}
	...
	jQuery.fn.carousel = function() {
		this.each(function() {
			...
			panels.css("position", "relative").each(function() {
				size += cfg.vertical ? jQuery(this).outerHeight(true) : jQuery(this).outerWidth(true);
			});
			...
			jQuery.extend(api, {
				...
				adjustPosition: function(isBackward) {
					...
					// 调整 panels 到下一个视图中
					jQuery.each(theItem, function() {
						jQuery(this).css(direction, isBackward ? -size : size + "px");
					});
				},
				resetPosition: function(isBackward) {
					var theItem = isBackward ? lastItem : firstItem;
...
;(function(jQuery) {
	...
	function wheelHandler( event ) {
		switch ( event.type ){
			// FF2
			case "mousemove": 
				return jQuery.extend( event.data, { // 存储正确的属性
					clientX: event.clientX, clientY: event.clientY,
					pageX: event.pageX, pageY: event.pageY
				});
			// firefox	
			case "DOMMouseScroll": 
				jQuery.extend( event, event.data );
 ...
 * 淡隐淡现
**/
jQuery.switchable.addEffect("fade", function(i, done) {
	var self = this,
		cfg = self.getCfg(),
 ...
 * </container>
**/
jQuery.switchable.addEffect("scroll", function(i, done) {
	var self = this,
		cfg = self.getCfg(),
		thisItem = self.getVisiblePanel(i),
